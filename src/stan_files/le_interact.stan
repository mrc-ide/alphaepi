...
transformed parameters{
  ...
  row_vector[nk_incrate_age] coef_incrate_age[NSITES];
  ...
  for (i in 1:NSITES) {
    coef_incrate_time[i] = row_means(coef_incrate_time_age[i]);
    coef_incrate_age[i] = col_means(coef_incrate_time_age[i]); 
  }
  
  for (j in 1:NSITES) {
    for(i in 1:nk_natmx_age)
      if (i < fixcoef_natmx_age){
        coef_natmx_age[j,i] = param_natmx_age[j,i];
      } else if (i == fixcoef_natmx_age) {
        coef_natmx_age[j,i] = -sum(param_natmx_age[j]);
      } else {
        coef_natmx_age[j,i] = param_natmx_age[j,i-1];
      }
  }

  // Informative prior derived based on Todd et al. (2007) Weibull regression.
  for (i in 1:NSITES) {
    hivsurv_scale_b0[i] = 0.25*hivsurv_scale_b0_centered[i] + 2.55; // prior ~normal(2.55, 0.25)
    hivsurv_scale_b1[i] = 0.05*hivsurv_scale_b1_centered[i] + -0.2; // prior ~normal(-0.2, 0.05)
  }
}
model {
  ...
  {
    matrix[nk_incrate_time, nk_incrate_age] resid_incrate_time_age; // check that this is ok not indexed by NSITES
    vector[nk_incrate_time*nk_incrate_age] vec_resid_incrate_time_age[NSITES];
    int ind_art_x_s;
    int ind_art_x_e;
    int ind_art_y_s;
    int ind_art_y_e;
    ...
    for(k in 1:NSITES) {
      for(j in 1:nk_incrate_age)
        for(i in 1:nk_incrate_time)
          resid_incrate_time_age[i,j] = coef_incrate_time_age[k,i,j] - coef_incrate_time[k,i] - coef_incrate_age[k,j];
      vec_resid_incrate_time_age[k] = to_vector(resid_incrate_time_age);
      target += -(nk_incrate_time-1)*(nk_incrate_age-1)*log(sigma_incrate_time_age) -
		       1/(2*sigma_incrate_time_age*sigma_incrate_time_age) * (vec_resid_incrate_time_age[k]' * Pcar_prec_incrate * vec_resid_incrate_time_age[k]);
      D_incrate_time * coef_incrate_time[k] ~ normal(0, sigma_incrate_time);
      D_incrate_age * to_vector(coef_incrate_age[k]) ~ normal(0, sigma_incrate_age);
  ...
  for (i in 1:NSITES) {
    ...
    hivsurv_scale_b0_centered[i] ~ normal(0, 1); 
    hivsurv_scale_b1_centered[i] ~ normal(0, 1);  
    // Informative prior on Weibull shape parameter -- mean 2, 95% quantiles (1.0, 3.3)
    hivsurv_shape[i] ~ gamma(12.0, 6.0);
  }
